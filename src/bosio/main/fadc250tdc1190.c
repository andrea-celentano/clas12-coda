...
void
FADC250DECODE(unsigned int *tdcbuf, int rlen, unsigned int *dabufp)
{
  int ii, jj, iii, itdcbuf;
  unsigned int *tdc;
      ...
      while(ii<rlen)
      {
        ...
        if( ((tdc[ii]>>27)&0x1F) == 0x10)
        {
		  ...
		  printf("[%3d] BLOCK HEADER: slot %d, nevents %d, block number %d\n",ii,
				   a_slot1,a_nevents,a_blocknumber);
#endif
		  ii++;
        }
        ...
        else if( ((tdc[ii]>>27)&0x1F) == 0x12)
        {
          ...
          while( ((tdc[ii]>>31)&0x1) == 0 )
		  {
            a_triggernumber = (tdc[ii]&0x7FFFFFF);
#ifdef DEBUG
            printf("   [%3d] EVENT HEADER 2: trigger number %d -> SERIOUS ERROR !!!\n",ii,
              a_triggernumber);
#endif
            ii++;
		  }
...
#ifdef DEBUG
		  printf("[%3d] WINDOW RAW DATA: channel %d, window width %d (a_id=0x%08x)\n",ii,
				   a_channel,a_windowwidth,a_id);
#endif
          *dabufp ++ = a_id;
		  ii++;
          while( ((tdc[ii]>>31)&0x1) == 0 && ii<rlen )
        ...
        {
          printf("[%3d] ERROR: in FADC data format 0x%08x\n",ii,
            (int)tdc[ii]);
          ii++;
        }
...
int iev_spect=0, iev_START=1, iev_STOP=2000000;
...
char *HBOOKfile = "fadc250tdc1190.his";
...
printf("===== Event no. %d\n",iev);
    ...
    if((ind1=bosNlink(bcs_.iw,"RC17",0)) > 0)
    {
...
{
  unsigned int dabufp[10000];
  b32+= 20;
  FADC250DECODE(b32, nw-20, dabufp);
}
