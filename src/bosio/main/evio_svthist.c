...
main(int argc, char **argv)
{
  ...
  unsigned short *b16;
  unsigned char *b08;
  int trig,chan,fpga,apv,hybrid,islot,ichip;
  ...
  x2 = 128.;
  ww = 0.;
  for(islot=3; islot<=4; islot++)
  {
    for(ichip=0; ichip<=7; ichip++)
    {
      idn = 2000+islot*10+ichip;
      sprintf(title,"sl%02dchip%1d",islot,ichip);
      hbook1_(&idn,title,&nbins,&x1,&x2,&ww,strlen(title));
    }
  }
	      ...
	      if(word & 0x80000000)
	      {
	        ...
	        switch(type)
	        {
		      case DATA_TYPE_BLKHDR:
                ...
                islot = (word>>22)&0x1f;
#ifdef DEBUG
		        printf(" {BLKHDR} SLOTID: %d", islot);fflush(stdout);
