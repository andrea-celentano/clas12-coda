...
typedef struct evfilestruct {
  ...
  int   blkNumDiff;      /**< When reading, the difference between blknum read in and
                          *   the expected (sequential) value. Used in debugmessage. */
  ...
  /* socket stuff */
  int   sockFd;          /**< socket file descriptor if reading/writing from/tosocket. */

  /* block stuff */
  int   blkSizeTarget;   /**< target size of block in 32 bit words (including blockheader). */
 ...
 *      Start         = offset to first event header in block relative to start ofblock
 ...
 *                           If a dictionary is included as the first event, bit #9is set (=1)
 *      Magic #            = magic number (0xc0da0100) used to check endianness
 *
 *
 *
...
#define EV_HD_BLKSIZ 0        /**< Position of blk hdr word for size of block in 32-bit
words. */
#define EV_HD_BLKNUM 1        /**< Position of blk hdr word for block number, starting at
0. */
#define EV_HD_HDSIZ  2        /**< Position of blk hdr word for size of header in 32-bitwords (=8). */
#define EV_HD_COUNT  3  /**< Position of blk hdr word for number of events in block(version 4+). */
#define EV_HD_START  3  /**< Position of blk hdr word for first start of event inthis block (ver 1-3). */
#define EV_HD_USED   4        /**< Position of blk hdr word for number of words used inblock (<= BLKSIZ) (ver 1-3). */
...
#define EV_HD_MAGIC  7        /**< Position of blk hdr word for magic number for endiannesstracking. */
...
#ifdef DEBUG
        fprintf(stderr,"evGetNewBuffer: block # read(%d) is different than
expected(%d)\n",
...
#ifdef DEBUG
        fprintf(stderr,"evGetNewBuffer: increase internal buffer size to %d
bytes\n", 4*a->blksiz);
#endif
 ...
 *                              or is smaller than minimum allowed size (header size
+ 1K);
