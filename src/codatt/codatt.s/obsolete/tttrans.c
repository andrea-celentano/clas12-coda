...
int mynev;
#else
extern int mynev; /*local event counter*/
#endif
...
if(bufout[2]!=NWBOS) printf("ERR in TT_Main: bufout[2]=%d (0x%08x)\n",bufout[2],bufout);
  ...
  printf("TT_Main: Ttp structure at 0x%8.8X\n",ttp);
  printf("TT_Main: bufout[0]=%i --> processing for ROC#%i(%i)\n",
         bufout[0],ROC,pid);
#endif
  ...
  else
  {
	  ...
	  */
      ...
      while((long *)fb < (&bufin[1] + bufin[0]))
      {
		...
		*/
        ...
        {
          printf("TT_Main: ERROR in bosMgetname()\n");
          goto exit;
		}
		else
        {
          ...
          if(lfmt > 0)
            ...
            printf("TT_Main: ERROR: lfmt=%d\n",lfmt);

          /*TT_Main: printf("id=%d nr=%d count=%x hex (%d decimal) -> name=>%s<\n",
          ...
          {
            /*TT_Main: printf("passing bank: nama=>%4.4s< nr=%d nw=%d ncol=%d nrow=%d\n",
		      &nama,nr,count,ncol,nrow);*/
            if( (ind = bosNopen(iw,nama,nr,ncol,nrow)) <= 0 )
            {
              printf("TT_Main: ERROR: bosNopen returns %d >%4.4s< nr=%d nw=%d\n",
    ...
    {
#ifdef DEBUG
	  printf("TT_Main: not empty CODA fragment\n");
#endif
      if((ind = bosMlink(jw,rcname,0)) > 0) /* get raw data bank */
      {
#ifdef DEBUG
	    printf("TT_Main: got raw data\n");
#endif
        if(WANNARAW)
        {
          len += TT_MakeRawBank(&jw[ind],bufout);
#ifdef DEBUG
	      printf("TT_Main: making raw bank, len=%d\n",len);
#endif
		}
        if(!ttp->notrans)
        {
