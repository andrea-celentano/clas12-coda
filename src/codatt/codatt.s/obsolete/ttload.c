...
/* local prototypes */
static void   *TT_OpenTable(int);
static TTRPtr  TT_GetRow(void *);
static int     TT_CloseTable(void *);
static TTSPtr  TT_Alloc(void);
static void    TT_Clean(TTSPtr);
static FILE   *TT_OpenTabFile(int);
static int     TT_ProcessRow(TTRPtr);
static void    TT_PrintSlot(int);
static void    TT_PrintSlots(void);
static int     TT_GetTTStatistics(void);
...
TTSPtr 
TT_LoadROCTT(int roc, TTSPtr TTP)
{
  void *tth;
  int    i;
  TTRPtr r;
  ...
  if(ttpwork == NULL) /* TT Struct not yet allocated */
  {
	...
	if( (ttpwork=TT_Alloc()) == NULL ) /* Can't alloc */
    {
  ...
  if ( (tth=TT_OpenTable(roc)) == NULL) /* Fails open Translation table */
  {
    ...
    do 
    {
      ...
      if (s == NULL)
      {
        if (ferror((FILE*)tth)) /* check for errors */
        {
          ...
          perror("TT_GetRow: Error in fgets()");
        }
  ...
  for(sl=FIRST_SLOT; sl<=LAST_SLOT; sl++)
  {
	...
	for(ch=0; ch<MAXCHANNEL; ch++)
    {
      if(ttpwork->id[sl][ch] != 0 || ttpwork->id2[sl][0][ch] != 0 || ttpwork->id2[sl][1][ch] != 0)
	  {
        ...
        for(i=0; i<4; i++) name[i] = chara[i];
        ttpwork->nplace[sl][ch] = bosMgetncol(name);
