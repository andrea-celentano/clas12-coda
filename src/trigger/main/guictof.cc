...
// guictof.cxx: gui for CTOF v1495-based trigger
...
#include "CTOFTriggerBoardRegs.h"
...
GUIMainFrame::GUIMainFrame(const TGWindow *p, UInt_t w, UInt_t h, char *host) : TGMainFrame(p, w, h)
{
   ...
   fMenuDock->SetWindowName("GUICTOF Menu"); // ???
   ...
   // menu item 'CTOF'
   fMenuCTOF = new TGPopupMenu(fClient->GetRoot());
   fMenuCTOF->AddLabel("CTOF monitoring and control");
   fMenuCTOF->AddSeparator();
   fMenuCTOF->AddEntry("&Registers", M_REGISTERS);
   fMenuCTOF->AddEntry("&Delays", M_DELAYS);
   fMenuCTOF->AddEntry("&Scalers", M_SCALERS);
   fMenuCTOF->AddEntry("&Dsc2", M_DSC2);
   fMenuCTOF->AddEntry("&Scope (ASCII)", M_SCOPE_ASCII);
   fMenuCTOF->AddEntry("&Scope (Canvas)", M_SCOPE_CANVAS);
   ...
   fMenuBar->AddPopup("&CTOF", fMenuCTOF, fMenuBarItemLayout);
...
Bool_t GUIMainFrame::ProcessMessage(Long_t msg, Long_t parm1, Long_t)
{
  ...
  UInt_t tmp, *buf;
   ...
   UInt_t board_addr[2];
   board_addr[0] = CTOF_BOARD_ADDRESS_1;
   if(CTOF_BOARD_ADDRESS_1 != CTOF_BOARD_ADDRESS_2) board_addr[1] = CTOF_BOARD_ADDRESS_2;
   else                                             board_addr[1] = 0;
   ...
   switch (GET_MSG(msg)) {
      ...
      case kC_COMMAND:
		...
		ret = tcp->Read16((unsigned int)0x11A0800C, &sval);
        printf("ret=%d, VME FIRMWARE val=0x%04x\n",ret,sval);
				  
		unsigned int val = 0xFBFBFBFB;
		ret = tcp->Read32((unsigned int)0x11A01000, &val);
  ...
  printf("222: ret=%d, len=%d\n",ret,len);fflush(stdout);
					   ...
					   UInt_t w, UInt_t h, UInt_t options) : TGTransientFrame(p, main, w, h, options)
{
  ...
  fMain = main; // remember mainframe
   ...
   {
     // U1
     if(jj % 2) sprintf(buff1, "  U%02d ->",(jj/2)+1);
     else       sprintf(buff1, "  D%02d ->",(jj/2)+1);
   ...
   {
     // V1
     if(jj % 2) sprintf(buff1, "  U%02d ->",(jj/2)+1+16);
     else       sprintf(buff1, "  D%02d ->",(jj/2)+1+16);
   ...
   {
     // W1
     if(jj % 2) sprintf(buff1, "  U%02d ->",(jj/2)+1+32);
     else       sprintf(buff1, "  D%02d ->",(jj/2)+1+32);
...
void ScalersDlg::FillHistos()
{
   ...
   if (!fHpxS1) {
      fHpxS1 = new TH1F("hpxS1","Cart1 scalers",16,0.,16.);
      fHpxS2 = new TH1F("hpxS2","Cart2 scalers",16,0.,16.);
      fHpxS3 = new TH1F("hpxS3","Cart3 scalers",16,0.,16.);
      fHpxS4 = new TH1F("hpxS3","Cart4 scalers",16,0.,16.);
      fHpxS5 = new TH1F("hpxS3","Cart5 scalers",16,0.,16.);
      fHpxS6 = new TH1F("hpxS3","Cart6 scalers",16,0.,16.);
...
#ifdef OLDOLD
	tcpvme->VMEWrite32(CTOF_BOARD_ADDRESS_1 + CTOF_ENABLE_SCALERS, 0x0000, TRUE);
	tcpvme->VMEWrite32(CTOF_BOARD_ADDRESS_2 + CTOF_ENABLE_SCALERS, 0x0000, TRUE);

	for(jj=0; jj<46; jj++) tcpvme->VMERead32(CTOF_BOARD_ADDRESS_1 + CTOF_U_SCALER_BASE + jj*4, &U1[jj], TRUE);
	for(jj=0; jj<46; jj++) tcpvme->VMERead32(CTOF_BOARD_ADDRESS_1 + CTOF_V_SCALER_BASE + jj*4, &V1[jj], TRUE);
	for(jj=0; jj<46; jj++) tcpvme->VMERead32(CTOF_BOARD_ADDRESS_1 + CTOF_W_SCALER_BASE + jj*4, &W1[jj], TRUE);	
	for(jj=0; jj<46; jj++) tcpvme->VMERead32(CTOF_BOARD_ADDRESS_2 + CTOF_U_SCALER_BASE + jj*4, &U2[jj], TRUE);
	for(jj=0; jj<46; jj++) tcpvme->VMERead32(CTOF_BOARD_ADDRESS_2 + CTOF_V_SCALER_BASE + jj*4, &V2[jj], TRUE);
	for(jj=0; jj<46; jj++) tcpvme->VMERead32(CTOF_BOARD_ADDRESS_2 + CTOF_W_SCALER_BASE + jj*4, &W2[jj], TRUE);	
	
    tcpvme->VMERead32(CTOF_BOARD_ADDRESS_1 + CTOF_REF_SCALER, &REF1, TRUE);
    tcpvme->VMERead32(CTOF_BOARD_ADDRESS_2 + CTOF_REF_SCALER, &REF2, TRUE);

    UInt_t revision;
    tcpvme->VMERead32(CTOF_BOARD_ADDRESS_1 + CTOF_FW_REVISION, &revision, TRUE);
    printf("ReadVME: revision=0x%08x\n",revision);

    printf("ReadVME: REF1=0x%08x REF2=0x%08x\n",REF1,REF2);
    printf("ReadVME: 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x\n",
      U1[0],U1[1],U1[2],U1[3],U1[4],U1[5],U1[6],U1[7],U1[8],U1[9]);

	tcpvme->VMEWrite32(CTOF_BOARD_ADDRESS_1 + CTOF_ENABLE_SCALERS, 0x0001, TRUE);
	tcpvme->VMEWrite32(CTOF_BOARD_ADDRESS_2 + CTOF_ENABLE_SCALERS, 0x0001, TRUE);
#else
    tmp = 0x0000;
	tcp->Write32(CTOF_BOARD_ADDRESS_1 + CTOF_ENABLE_SCALERS, &tmp);
	tcp->Write32(CTOF_BOARD_ADDRESS_2 + CTOF_ENABLE_SCALERS, &tmp);

	for(jj=0; jj<46; jj++) tcp->Read32(CTOF_BOARD_ADDRESS_1 + CTOF_U_SCALER_BASE + jj*4, &U1[jj]);
	for(jj=0; jj<46; jj++) tcp->Read32(CTOF_BOARD_ADDRESS_1 + CTOF_V_SCALER_BASE + jj*4, &V1[jj]);
	for(jj=0; jj<46; jj++) tcp->Read32(CTOF_BOARD_ADDRESS_1 + CTOF_W_SCALER_BASE + jj*4, &W1[jj]);	
	for(jj=0; jj<46; jj++) tcp->Read32(CTOF_BOARD_ADDRESS_2 + CTOF_U_SCALER_BASE + jj*4, &U2[jj]);
	for(jj=0; jj<46; jj++) tcp->Read32(CTOF_BOARD_ADDRESS_2 + CTOF_V_SCALER_BASE + jj*4, &V2[jj]);
	for(jj=0; jj<46; jj++) tcp->Read32(CTOF_BOARD_ADDRESS_2 + CTOF_W_SCALER_BASE + jj*4, &W2[jj]);	
	
    tcp->Read32(CTOF_BOARD_ADDRESS_1 + CTOF_REF_SCALER, &REF1);
    tcp->Read32(CTOF_BOARD_ADDRESS_2 + CTOF_REF_SCALER, &REF2);

    UInt_t revision;
    tcp->Read32(CTOF_BOARD_ADDRESS_1 + CTOF_FW_REVISION, &revision,1,0);
    printf("ReadVME: revision=0x%08x\n",revision);
...
void Dsc2Dlg::ReadVME()
{
  ...
  addr[0]  = CTOF_DSC2_ADDRESS_SLOT2;
  addr[1]  = CTOF_DSC2_ADDRESS_SLOT4;
  addr[2]  = CTOF_DSC2_ADDRESS_SLOT5;
  addr[3]  = CTOF_DSC2_ADDRESS_SLOT7;
  addr[4]  = CTOF_DSC2_ADDRESS_SLOT8;
  addr[5]  = CTOF_DSC2_ADDRESS_SLOT10;
  addr[6]  = CTOF_DSC2_ADDRESS_SLOT12;
  addr[7]  = CTOF_DSC2_ADDRESS_SLOT14;
  addr[8]  = CTOF_DSC2_ADDRESS_SLOT15;
  addr[9]  = CTOF_DSC2_ADDRESS_SLOT17;
  addr[10] = CTOF_DSC2_ADDRESS_SLOT18;
  addr[11] = CTOF_DSC2_ADDRESS_SLOT20;
...
#endif
  {
...
#ifdef OLDOLD
    for(ii=0; ii<NDSC; ii++) tcpvme->VMEWrite32(addr[ii] + CTOF_DSC2_SCALER_GATE, 0x0004, FALSE);

    for(ii=0; ii<NDSC; ii++) tcpvme->VMEWrite32(addr[ii] + CTOF_DSC2_SCALER_LATCH, 0x0000, FALSE);
    ...
    for(ii=0; ii<NDSC; ii++) tcpvme->VMERead32(addr[ii] + CTOF_DSC2_SCALER_REF, &ref[ii], FALSE);



    for(jj=0; jj<16; jj++) {tcpvme->VMERead32(addr[0] + CTOF_DSC2_SCALER_BASE + jj*4,     &U1[jj],    FALSE); refU1[jj]    = ref[0];}
    for(jj=0; jj<16; jj++) {tcpvme->VMERead32(addr[1] + CTOF_DSC2_SCALER_BASE + jj*4,     &U1[jj+16], FALSE); refU1[jj+16] = ref[1];}
    for(jj=0; jj<16; jj++) {tcpvme->VMERead32(addr[2] + CTOF_DSC2_SCALER_BASE + jj*4,     &U1[jj+32], FALSE); refU1[jj+32] = ref[2];}
    for(jj=0; jj<16; jj++) {tcpvme->VMERead32(addr[3] + CTOF_DSC2_SCALER_BASE + jj*4,     &U1[jj+48], FALSE); refU1[jj+48] = ref[3];}
    for(jj=0; jj<4; jj++)  {tcpvme->VMERead32(addr[4] + CTOF_DSC2_SCALER_BASE + jj*4,     &U1[jj+64], FALSE); refU1[jj+64] = ref[4];}

    for(jj=0; jj<12; jj++) {tcpvme->VMERead32(addr[4] + CTOF_DSC2_SCALER_BASE + (jj+4)*4, &V1[jj],    FALSE); refV1[jj]    = ref[4];}
    for(jj=0; jj<16; jj++) {tcpvme->VMERead32(addr[5] + CTOF_DSC2_SCALER_BASE + jj*4,     &V1[jj+12], FALSE); refV1[jj+12] = ref[5];}
    for(jj=0; jj<16; jj++) {tcpvme->VMERead32(addr[6] + CTOF_DSC2_SCALER_BASE + jj*4,     &V1[jj+28], FALSE); refV1[jj+28] = ref[6];}
    for(jj=0; jj<16; jj++) {tcpvme->VMERead32(addr[7] + CTOF_DSC2_SCALER_BASE + jj*4,     &V1[jj+44], FALSE); refV1[jj+44] = ref[7];}
    for(jj=0; jj<2; jj++)  {tcpvme->VMERead32(addr[8] + CTOF_DSC2_SCALER_BASE + jj*4,     &V1[jj+60], FALSE); refV1[jj+60] = ref[8];}

    for(jj=0; jj<14; jj++) {tcpvme->VMERead32(addr[8] + CTOF_DSC2_SCALER_BASE + (jj+2)*4, &W1[jj],    FALSE); refW1[jj]    = ref[8];}
    for(jj=0; jj<16; jj++) {tcpvme->VMERead32(addr[9] + CTOF_DSC2_SCALER_BASE + jj*4,     &W1[jj+14], FALSE); refW1[jj+14] = ref[9];}
    for(jj=0; jj<16; jj++) {tcpvme->VMERead32(addr[10]+ CTOF_DSC2_SCALER_BASE + jj*4,     &W1[jj+30], FALSE); refW1[jj+30] = ref[10];}
    for(jj=0; jj<16; jj++) {tcpvme->VMERead32(addr[11]+ CTOF_DSC2_SCALER_BASE + jj*4,     &W1[jj+46], FALSE); refW1[jj+46] = ref[11];}
#else
    tmp = 0x0004;
    for(ii=0; ii<NDSC; ii++) tcp->Write32(addr[ii] + CTOF_DSC2_SCALER_GATE, &tmp);

    tmp = 0x0000;
    for(ii=0; ii<NDSC; ii++) tcp->Write32(addr[ii] + CTOF_DSC2_SCALER_LATCH, &tmp);
    ...
    for(ii=0; ii<NDSC; ii++) tcp->Read32(addr[ii] + CTOF_DSC2_SCALER_REF, &ref[ii]);



    for(jj=0; jj<16; jj++) {tcp->Read32(addr[0] + CTOF_DSC2_SCALER_BASE + jj*4,     &U1[jj]   ); refU1[jj]    = ref[0];}
    for(jj=0; jj<16; jj++) {tcp->Read32(addr[1] + CTOF_DSC2_SCALER_BASE + jj*4,     &U1[jj+16]); refU1[jj+16] = ref[1];}
    for(jj=0; jj<16; jj++) {tcp->Read32(addr[2] + CTOF_DSC2_SCALER_BASE + jj*4,     &U1[jj+32]); refU1[jj+32] = ref[2];}
    for(jj=0; jj<16; jj++) {tcp->Read32(addr[3] + CTOF_DSC2_SCALER_BASE + jj*4,     &U1[jj+48]); refU1[jj+48] = ref[3];}
    for(jj=0; jj<4; jj++)  {tcp->Read32(addr[4] + CTOF_DSC2_SCALER_BASE + jj*4,     &U1[jj+64]); refU1[jj+64] = ref[4];}

    for(jj=0; jj<12; jj++) {tcp->Read32(addr[4] + CTOF_DSC2_SCALER_BASE + (jj+4)*4, &V1[jj]   ); refV1[jj]    = ref[4];}
    for(jj=0; jj<16; jj++) {tcp->Read32(addr[5] + CTOF_DSC2_SCALER_BASE + jj*4,     &V1[jj+12]); refV1[jj+12] = ref[5];}
    for(jj=0; jj<16; jj++) {tcp->Read32(addr[6] + CTOF_DSC2_SCALER_BASE + jj*4,     &V1[jj+28]); refV1[jj+28] = ref[6];}
    for(jj=0; jj<16; jj++) {tcp->Read32(addr[7] + CTOF_DSC2_SCALER_BASE + jj*4,     &V1[jj+44]); refV1[jj+44] = ref[7];}
    for(jj=0; jj<2; jj++)  {tcp->Read32(addr[8] + CTOF_DSC2_SCALER_BASE + jj*4,     &V1[jj+60]); refV1[jj+60] = ref[8];}

    for(jj=0; jj<14; jj++) {tcp->Read32(addr[8] + CTOF_DSC2_SCALER_BASE + (jj+2)*4, &W1[jj]   ); refW1[jj]    = ref[8];}
    for(jj=0; jj<16; jj++) {tcp->Read32(addr[9] + CTOF_DSC2_SCALER_BASE + jj*4,     &W1[jj+14]); refW1[jj+14] = ref[9];}
    for(jj=0; jj<16; jj++) {tcp->Read32(addr[10]+ CTOF_DSC2_SCALER_BASE + jj*4,     &W1[jj+30]); refW1[jj+30] = ref[10];}
    for(jj=0; jj<16; jj++) {tcp->Read32(addr[11]+ CTOF_DSC2_SCALER_BASE + jj*4,     &W1[jj+46]); refW1[jj+46] = ref[11];}
#endif

	printf("Scalers: ref1=%d, U1=%d %d %d (address=0x%08x)\n",
      ref[0],U1[0],U1[1],U1[2],addr[0] + CTOF_DSC2_SCALER_BASE);
...
#ifdef OLDOLD
	for(jj=0; jj<46; jj++) tcpvme->VMERead32(CTOF_BOARD_ADDRESS_1 + CTOF_U_DELAY_BASE + jj*4, &U1[jj], TRUE);
	for(jj=0; jj<46; jj++) tcpvme->VMERead32(CTOF_BOARD_ADDRESS_1 + CTOF_V_DELAY_BASE + jj*4, &V1[jj], TRUE);
	for(jj=0; jj<46; jj++) tcpvme->VMERead32(CTOF_BOARD_ADDRESS_1 + CTOF_W_DELAY_BASE + jj*4, &W1[jj], TRUE);
#else
	for(jj=0; jj<46; jj++) tcp->Read32(CTOF_BOARD_ADDRESS_1 + CTOF_U_DELAY_BASE + jj*4, &U1[jj]);
	for(jj=0; jj<46; jj++) tcp->Read32(CTOF_BOARD_ADDRESS_1 + CTOF_V_DELAY_BASE + jj*4, &V1[jj]);
	for(jj=0; jj<46; jj++) tcp->Read32(CTOF_BOARD_ADDRESS_1 + CTOF_W_DELAY_BASE + jj*4, &W1[jj]);
#endif
...
#ifdef OLDOLD
    for(jj=0; jj<46; jj++) if(U1[jj] != U1GUI[jj]) tcpvme->VMEWrite32(CTOF_BOARD_ADDRESS_1 + CTOF_U_DELAY_BASE + jj*4, U1GUI[jj], TRUE);
    for(jj=0; jj<46; jj++) if(V1[jj] != V1GUI[jj]) tcpvme->VMEWrite32(CTOF_BOARD_ADDRESS_1 + CTOF_V_DELAY_BASE + jj*4, V1GUI[jj], TRUE);
    for(jj=0; jj<46; jj++) if(W1[jj] != W1GUI[jj]) tcpvme->VMEWrite32(CTOF_BOARD_ADDRESS_1 + CTOF_W_DELAY_BASE + jj*4, W1GUI[jj], TRUE);
#else
    for(jj=0; jj<46; jj++) if(U1[jj] != U1GUI[jj]) tcp->Write32(CTOF_BOARD_ADDRESS_1 + CTOF_U_DELAY_BASE + jj*4, &U1GUI[jj]);
    for(jj=0; jj<46; jj++) if(V1[jj] != V1GUI[jj]) tcp->Write32(CTOF_BOARD_ADDRESS_1 + CTOF_V_DELAY_BASE + jj*4, &V1GUI[jj]);
    for(jj=0; jj<46; jj++) if(W1[jj] != W1GUI[jj]) tcp->Write32(CTOF_BOARD_ADDRESS_1 + CTOF_W_DELAY_BASE + jj*4, &W1GUI[jj]);
#endif
  }
