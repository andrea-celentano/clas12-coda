...
class PCALMainFrame : public TGMainFrame {
...
public:
   ...
   PCALMainFrame(const TGWindow *p, UInt_t w, UInt_t h, char *host);
   virtual ~PCALMainFrame();
...
private:
   ...
   Int_t                fLastEntry;
   Bool_t               fFillHistos;
   TH1F                *fHpxU, *fHpxV, *fHpxW;

   TGTextBuffer *tbufU1[68], *tbufV1[62], *tbufW1[62];
   TGTextEntry *tentU1[68], *tentV1[62], *tentW1[62];
...
public:
   ScalersDlg(const TGWindow *p, PCALMainFrame *main, UInt_t w, UInt_t h,
...
private:
   ...
   TGTextBuffer *tbufU1[NU], *tbufV1[NV], *tbufW1[NW];
   TGTextEntry *tentU1[NU], *tentV1[NV], *tentW1[NW];
...
public:
   Dsc2Dlg(const TGWindow *p, PCALMainFrame *main, UInt_t w, UInt_t h,
               UInt_t options = kVerticalFrame);
   virtual ~Dsc2Dlg();
...
public:
   DelaysDlg(const TGWindow *p, PCALMainFrame *main, UInt_t w, UInt_t h,
               UInt_t options = kVerticalFrame);
   virtual ~DelaysDlg();
   virtual void CloseWindow();
   virtual Bool_t ProcessMessage(Long_t msg, Long_t parm1, Long_t parm2);
...
{
private:
  PCALMainFrame   *fPCALMainFrame;   //display to which this timer belongs

public:
  MyTimer(PCALMainFrame *m, Long_t ms);
  Bool_t  Notify();
};
