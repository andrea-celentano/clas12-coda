...
*/
  ...
  else
  {
    ...
    erp->current_file = ckalloc((strlen(erp->filename)+200));
	printf("par1: >%s<\n",erp->filename);
    printf("par2: %d\n",object->runNumber);
    printf("par3: %d\n",erp->splitnb);
	printf("befor: >%s<\n",erp->current_file);
...
printf("CODA_write_event: error in et_events_get, status = ET_ERROR \n");
    ...
    /* if we got events ... */
	  ...
	  {
        int handle1;
        et_event_getlength(pe[ii], &len); /*get event length from et*/
		/*printf("len1=%d\n",len);*/
        memcpy((char *)hpsbuf, (char *)pe[ii]->pdata, len); /*copy event from et to the temporary buffer*/
        status = evOpenBuffer(pe[ii]->pdata, MAXBUF, "w", &handle1); /*open 'buffer' in et*/
        ...
        et_event_setlength(pe[ii], len); /*update event length in et*/
	  }
#endif
