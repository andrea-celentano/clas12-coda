...
/* v1190 start */
  ...
  printf("vmeUserLib: nboards=%d\n",nboards);
  for(ii=0; ii<nboards; ii++)
  {
    printf("vmeUserLib: tdcadr[%2d] = 0x%08x\n",ii,tdcadr[ii]);
  }
  ...
  if(nboards>0)
  {
    ...
    if(rocid ==      29) {tdc1190SetDefaultWindowWidth(1450); tdc1190SetDefaultWindowOffset(-4050);}
    else if(rocid == 28) {tdc1190SetDefaultWindowWidth(1450); tdc1190SetDefaultWindowOffset(-4050);}
    else if(rocid == 17) {tdc1190SetDefaultWindowWidth(1950); tdc1190SetDefaultWindowOffset(-4550);}
    else if(rocid == 20) {tdc1190SetDefaultWindowWidth(1200); tdc1190SetDefaultWindowOffset(-3800);}
    else if(rocid == 21) {tdc1190SetDefaultWindowWidth(1000); tdc1190SetDefaultWindowOffset(-3600);}
    else if(rocid == 22) {tdc1190SetDefaultWindowWidth(1100); tdc1190SetDefaultWindowOffset(-3600);}
    else if(rocid == 27) {tdc1190SetDefaultWindowWidth(1500); tdc1190SetDefaultWindowOffset(-4500);}
    else if(rocid ==  0) {tdc1190SetDefaultWindowWidth(1500); tdc1190SetDefaultWindowOffset(-3600);}
    else                    {tdc1190SetDefaultWindowWidth(1500); tdc1190SetDefaultWindowOffset(-3900);}

    /* configuring all boards */
  ...
  if(rocid==20||rocid==21||rocid==22||rocid == 0)
  {
    for(ii=0; ii<nboards; ii++)
    {
	  ...
	  */
      tdata = 128;
      tdc1190SetEffectiveSizeOfReadoutFIFO(ii, tdata);
      tdata = tdc1190GetEffectiveSizeOfReadoutFIFO(ii);
...
vmeUserEnd()
{
  ...
  if(vmeIntCount > 0)
  {
    ...
    {
	  status = tdc1190ReadDone();
      if(status <= 0)
      {
        logMsg("BAD%d\n",status,2,3,4,5,6);
        for(jj=0; jj<nboards; jj++) rlenbuf[jj] = 0;
      }
      logMsg("vmeUserEnd: DMA status=%d\n",status,2,3,4,5,6);
...
#define MYPUTEVENT \
      status = tdc1190ReadDone(); \
      /*logMsg("tdc1190ReadDone: DMA status=%d\n",status,2,3,4,5,6);*/ \
      if(status <= 0) \
      { \
        logMsg("BAD%d\n",status,2,3,4,5,6); \
        for(jj=0; jj<nboards; jj++) rlenbuf[jj] = 0; \
...
logMsg("vmeUserTrigger reached: vmeIntCount=%d\n",vmeIntCount,2,3,4,5,6);
  */
  ...
  /*logMsg("vmeIntCount=%d\n",vmeIntCount,2,3,4,5,6);*/
  if(vmeIntCount > 1)
  {
    /*logMsg("syncFlag=%d type=%d\n",syncFlag,type,3,4,5,6);*/
    if(syncFlag==1)
    {
  ...
  else /* physics and physics (scheduled) sync events */
  {
    ...
    {
      /* start readout */
      status = tdc1190ReadListStart(tdcbuf, rlenbuf);
      if(status != OK) logMsg("ERROR in tdc1190ReadListStart() !!!\n",1,2,3,4,5,6);
    }
