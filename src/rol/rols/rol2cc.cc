...
/* rol2cc.c - default second readout list */
...
#define ROL_NAME__ "ROL2CC"
#define INIT_NAME rol2cc__init
...
*/
  ...
  for(ii=0; ii<nASIS; ii++)
  {
...

	{
	  ...
	  int offset[8][16][128];
      int noise[8][16][128];
	  zero_suppression((uint32_t *)datain, lenin/*bytes ??? words ?? */, (uint32_t *)dataout, offset, noise);
