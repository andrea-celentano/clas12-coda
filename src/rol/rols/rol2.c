...
static void __prestart() {
	...
	for (ii = 0; ii < NSLOTS; ii++) {
		if (tdctypebyslot[ii] == 1)
			NBsubtract[ii] = 9; /*v1190*/
		else if (tdctypebyslot[ii] == 2)
			NBsubtract[ii] = 9; /*v1290*/
		else if (tdctypebyslot[ii] == 3)
			NBsubtract[ii] = 5; /*v1290N*/
		else
			NBsubtract[ii] = 0; /*there is no board*/
	...
	for (ii = 0; ii < ntdcs; ii++)
		printf("ROL2:  tdc_id %d, slotnum %d\n", ii, slotnums[ii]);

#endif

	for (ii = 0; ii < NTRIGBITS; ii++)
		bitscalers[ii] = 0;
#ifdef SSIPC
				...
				} else if (((datain[ii] >> 27) & 0x1F) == 0x14) /*window raw data: remember channel#*/
				{
					...
					if (a_windowwidth != samplecount) {
						...
						sprintf(errmsg, "[%3d][%3d] ERROR1: a_windowwidth=%d != samplecount=%d (slot %d (%d %d ?), channel %d)\n", mynev,
								ii, a_windowwidth, samplecount, a_slot, a_slot_prev, a_slot2, a_channel);
						fflush(stdout);
					}
				...
				else if (((datain[ii] >> 27) & 0x1F) == 0x17) /* pulse integral */
				{
					...
					if (npulse_integral[m][a_slot][a_channel] != a_pulsenumber) {
						printf("ERROR: slot=%d channel=%d npulse_integral=%d != a_pulsenumber=%d\n", a_slot, a_channel,
								npulse_integral[m][a_slot][a_channel], a_pulsenumber);
					}
				...
				else if (((datain[ii] >> 27) & 0x1F) == 0x18) /* pulse time */
				{
					...
					if (npulse_time[m][a_slot][a_channel] != a_pulsenumber) {
						printf("ERROR: slot=%d channel=%d npulse_time=%d != a_pulsenumber=%d\n", a_slot, a_channel,
								npulse_time[m][a_slot][a_channel], a_pulsenumber);
					}
				...
				else if (((datain[ii] >> 27) & 0x1F) == 0x1a) /* Vm Vp */
				{
					...
					if (npulse_VmVp[m][a_slot][a_channel] != a_pulsenumber) {
						printf("ERROR: slot=%d channel=%d npulse_VmVp=%d != a_pulsenumber=%d\n", a_slot, a_channel,
								npulse_VmVp[m][a_slot][a_channel], a_pulsenumber);
					}
					...
					if (printing) /* printing only once at every event */
					{
						printf("[%3d][%3d] pass1 ERROR: in FADC data format 0x%08x (bits31-27=0x%02x), slot=%d (%d %d ?)\n", mynev, ii,
								(int) datain[ii], (datain[ii] >> 27) & 0x1F, a_slot, a_slot_prev, a_slot2);
...
//			while (ii < lenin) {
			...
			if (error) {
				printf("\n\n\n!!!!!!!!!!!!! ERROR FOUND - RECORD RAW BANK\n\n\n");
				{
					FILE *fd;
					char fname[256];
					sprintf(fname, "/tmp/v1725_error_event_%d.txt", mynev);
					fd = fopen(fname, "w");
					if (fd != NULL) {
						fprintf(fd, "%s\n", errmsg);
						for (ii = 0; ii < lenin; ii++)
			...
			while (ii < lenin) {
				...
				if (((datain[ii] >> 27) & 0x1F) == 0x10) /*block header*/
				{
					...
					for (iii = 0; iii < a_nevents; iii++) {
						...
						if (a_event_type == 0)
							printf("ERROR in TI event header word: event type = %d\n", a_event_type);
						if (((datain[ii] >> 16) & 0xFF) != 0x01)
							printf("ERROR in TI event header word (0x%02x)\n", ((datain[ii] >> 16) & 0xFF));
				...
				} else if (((datain[ii] >> 27) & 0x1F) == 0x11) /*block trailer*/
				{
					...
					printf("TI UNKNOWN data: [%3d] 0x%08x\n", ii, datain[ii]);
					{
						int jjj;
						printf("\n   Previous stuff\n");
						for (jjj = ii - 20; jjj < ii; jjj++)
							printf("          [%3d][%3d] 0x%08x\n", jjj, ii, datain[jjj]);
						for (jjj = ii; jjj < ii + 10; jjj++)
							printf("           [%3d][%3d] 0x%08x\n", jjj, ii, datain[jjj]);
				...
				if (((datain[ii] >> 27) & 0x1F) == 0) {
					...
					if (!(error_flag[tdcslot] % 100)) /* do not print for every event */
					{
						...
						printf(" ERR: event# %7d, slot# %2d, tdc# %02d, error_flags=0x%08x, err=0x%04x, lock=0x%04x\n", tdceventcount,
								tdcslot, (int) tdc14, (int) tdcerrorflags, ddd, lock);
			...
			while (ii < lenin) {
					...
					if (a_nwords != ((ii - iB[jj][nB[jj] - 1] + 1) - ndnv)) {
						...
						if (printing) {
							printf("[%3d][%3d] ERROR2 in SSP-RICH data: trailer #words=%d != actual #words=%d - ndnv=%d)\n", mynev, ii,
									a_nwords, ii - iB[jj][nB[jj] - 1] + 1, ndnv);
							printing = 0;
						}
					}
					if (ndnv > 0) {
				...
				} else if (((datain[ii] >> 27) & 0x1F) == 0x13) /*trigger time: remember timestamp*/
				{
					...
					while (((datain[ii] >> 31) & 0x1) == 0 && ii < lenin) /*must be one more word*/
					{
						...
						iii++;
						if (iii > 2)
							printf("ERROR1 in SSP-RICH: iii=%d\n", iii);
						ii++;
					}
				}
	...
	for (iev = 0; iev < nnE; iev++) {
		...
		for (jj = 0; jj < nbanks; jj++) /* loop over evio banks */
		{
			...
			else if (banktag[jj] == 0xe119) /* 0xe119 v1725 hardware format */
			{
				...
				for (ibl = 0; ibl < nB[jj]; ibl++) /*loop over blocks. There is only one.*/
				{
					a_channel_old = -1;
					...
					rlen = ii + lenE[jj][ibl][iev];
#ifdef DEBUG_V1725
					printf("0xe109: rlen is %i \n",rlen);
#endif
					while (ii < rlen) {
						/*First word: event size - don't need*/
						ii++;
						/*Second word: channel mask (first part), board id*/
						a_slot3 = ((datain[ii] >> 27) & 0x1F);
						a_chmask = ((datain[ii]) & 0xff);
						ii++;
						/*Third word: event counter, channel mask (second part)*/
						a_chmask = (a_chmask) + (((datain[ii] >> 24) & 0xff) << 8);
						...
						ii++;
						/*Fourth word: trigger time*/
						/*This will be written in EVIO as
						 *  *b64 = (((unsigned long long)a_trigtime[0])<<24) | a_trigtime[1];
						 *  However V1725 has a 32-bit trigger time
						 *  125 MHz frequency -> 1 tick = 16 ns
						 *  2^32 -> 17 s
						...
						/*Also, the number of samples per channel is always even, thus it is not possible to have, in the same word, samples from different channels*/
						a_activechannels = 0;
						for (iii = 0; iii < 16; iii++) {
							if (a_chmask & (0x1 << iii)) {
								a_activechannelslist[a_activechannels] = iii;
								a_activechannels++;
							}
						}
						...
						printf("0x%08x: before CCOPEN(1), dataout=0x%08x\n",b08,dataout);fflush(stdout);
#endif

						CCOPEN(0xe101, "c,i,l,N(c,Ns)", banknum);
						Nchan[0] = a_activechannels; //first N
						a_nwords = 0;
						iii = 0;
#ifdef DEBUG_V1725
