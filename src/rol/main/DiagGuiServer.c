...
static int nfadc, ndsc2_tcp, nvscm, nssp, nv1495, rflag, rmode;
...
static int vmeSetScalersReadInterval(int time) {
	if (time < 0)
		vmeScalersReadInterval = 0; /* 0 means no reads */
	else if (time > 100)
		vmeScalersReadInterval = 100;
	else
		vmeScalersReadInterval = time;
	...
	for (itype = 0; itype < SCALER_TYPE_MAX; itype++) {
		...
		} else if (itype == SCALER_TYPE_SSP) /* ssp scalers */
		{
			for (id = 0; id < nssp; id++) {
				...
				if (sspGetFirmwareType(slot) == SSP_CFG_SSPTYPE_HALLBRICH) {
					...
					for (fiber = 0; fiber < 32; fiber++) {
						...
						if (fibermask & (1 << fiber)) {
							...
							memcpy(&sspbuf[nw], maroc, RICH_CHAN_NUM * sizeof(int));
							nw += RICH_CHAN_NUM;
				...
				vmescalerslen[slot] = nw;
				for (ii = 0; ii < nw; ii++)
					vmescalers[slot][ii] = sspbuf[ii];
				/*printf("vmeScalersRead: nw=%d, vmescalers[slot][nw-2]=%d, vmescalers[slot][nw-1]=%d\n",nw,vmescalers[slot][nw-2],vmescalers[slot][nw-1]);*/
			}
		} //end if scaler SSP
		else if ((itype == SCALER_TYPE_V1495) && (nv1495 == 1)) {
			slot = v1495Slot();
			vmeBusLock();
			nw = 0;
			chmask=0xFFFFFFFF; /*32 channels, 32 logics, 32 triggers - last is the clock*/
...
/* return in the buf the threshold values of the type defined by 'type'
 ...
 or unknown */
int vmeGetBoardParams(int slot, int partype, int *buf, int *len) {
	int chan;
	...
	*len = 0;
	...
	if (vmescalersmap[slot] == SCALER_TYPE_DSC2) {
		if (partype == SCALER_PARTYPE_THRESHOLD) {
			...
			*len = 16;
			for (chan = 0; chan < 16; chan++)
				buf[chan] = dsc2GetThreshold(slot, chan, 1);
			vmeBusUnlock();
		} else if (partype == SCALER_PARTYPE_THRESHOLD2) {
			vmeBusLock();
			*len = 16;
			for (chan = 0; chan < 16; chan++)
				buf[chan] = dsc2GetThreshold(slot, chan, 2);
			vmeBusUnlock();
		...
		if (partype == SCALER_PARTYPE_THRESHOLD) {
			...
			*len = 16;
			for (chan = 0; chan < 16; chan++)
				buf[chan] = faGetChThreshold(slot, chan);
			vmeBusUnlock();
		} else if (partype == SCALER_PARTYPE_NCHANNELS) {
			*len = 1;
			buf[0] = 16;
		}
	} else if (vmescalersmap[slot] == SCALER_TYPE_V1495) {
		if (partype == SCALER_PARTYPE_THRESHOLD) {
			vmeBusLock();
			*len = 1;
			for (chan = 0; chan < 1; chan++)
				buf[chan] = 0;// faGetChThreshold(slot, chan);
			vmeBusUnlock();
		} else if (partype == SCALER_PARTYPE_NCHANNELS) {
			*len = 1;
			buf[0] = 32; /*HOW MANY CHANNELS HAS THE V1495 SCALERS??? 32 channels (channel/logic/scaler)*/
		}
	}
...
/* same for channel */
int vmeGetChannelParams(int slot, int channel, int partype, int *buf, int *len) {
...
int vmeSetChannelParams(int slot, int channel, int partype, int *buf, int len) {
	int err = 1;
...
static void vmeReadTask() {
	...
	printf("vmeReadTask starts \n");
	fflush(stdout);

	vmeSetScalersReadInterval(10); //read every 10 s
	...
	for (ii = 0; ii < ndsc2_tcp; ii++)
		if ((slot = dsc2Slot_tcp(ii)) > 0)
			vmescalersmap[slot] = SCALER_TYPE_DSC2;
	...
	for (ii = 0; ii < nfadc; ii++)
		if ((slot = faSlot(ii)) > 0)
			vmescalersmap[slot] = SCALER_TYPE_FADC250;
	...
	for (ii = 0; ii < nvscm; ii++)
		if ((slot = vscmSlot(ii)) > 0)
			vmescalersmap[slot] = SCALER_TYPE_VSCM;
	...
	/* fill map array with FADC's found */
	for (ii = 0; ii < nssp; ii++)
		if ((slot = sspSlot(ii)) > 0)
			vmescalersmap[slot] = SCALER_TYPE_SSP;
	...
	/*   V1495INIT */
	/**************/
	printf("Start V1495 initialization \n");
	iFlag = 0; /* base address */
	if (init_boards == 0) {
		/* skip initialization* */
		iFlag |= (1 << 16);
	}
	nv1495 = v1495Init(0xa00000, iFlag); /*Returns 0 if board found, otherwise something else*/
	if (nv1495 == 0) {
		nv1495 = 1;
		printf("Done V1495 initialization, nv1495: %d\n", nv1495);
	} else {
		printf("DiagGuiServer: error in v1495 initialization: %d \n", nv1495);
		nv1495 = 0;
	}
	/* fill map array with V1495's found */
	for (ii = 0; ii < nv1495; ii++) {
		if ((slot = v1495Slot(ii)) > 0) {
			vmescalersmap[slot] = SCALER_TYPE_V1495;
		}
	}
 ...
 */
int GetCrateMap(int *buf, int *len) {
	int slot, ii;

	*len = MAXBOARDS + 1;
	for (slot = 0; slot < (*len); slot++)
		buf[slot] = vmescalersmap[slot];
	/*
	 printf("\nGetCrateMap: *len=%d\n",*len);fflush(stdout);
...
int Vme_ReadScalers(Cmd_ReadScalers *pCmd_ReadScalers, Cmd_ReadScalers_Rsp *pCmd_ReadScalers_Rsp) {
	...
	for (ii = 0; ii < pCmd_ReadScalers_Rsp->cnt; ii++)
		pWr[ii] = vmescalers[slot][ii];
...
int Vme_ReadData(Cmd_ReadScalers *pCmd_ReadScalers, Cmd_ReadScalers_Rsp *pCmd_ReadScalers_Rsp) {
	int *pWr = pCmd_ReadScalers_Rsp->vals;
	int c = pCmd_ReadScalers->cnt; /* count from input message - not used */
	int size/* = 4+4*c*/;
	int nw, slot, ii;
...
int Vme_GetBoardParams(Cmd_GetBoardParams *pCmd, Cmd_GetBoardParams_Rsp *pCmd_Rsp) {
...
int Vme_GetChannelParams(Cmd_GetChannelParams *pCmd, Cmd_GetChannelParams_Rsp *pCmd_Rsp) {
	int *pWr = pCmd_Rsp->vals;
	int size = 4;
	int slot = pCmd->slot;
...
int Vme_SetChannelParams(Cmd_SetChannelParams *pCmd) {
	int *pRd = pCmd->vals;
	int size = 4;
	int slot = pCmd->slot;
	int channel = pCmd->channel;
...
int Vme_GetCrateMap(Cmd_GetCrateMap *pCmd, Cmd_GetCrateMap_Rsp *pCmd_Rsp) {
	int *pWr = pCmd_Rsp->vals;
	...
	for (ii = 0; ii < pCmd_Rsp->nslots; ii++)
		printf("Vme_GetCrateMap: [%2d] 0x%08x\n", ii, pWr[ii]);
...
int main(int argc, char *argv[]) {
	...
	if (init_boards)
		vmeCheckMutexHealth(1); /* kiils mutex if exist */
...
// Run socket server
	...
	printf("Addr of Read16: 0x%08x\n", gCrateServerCBFcn.Read16);
	fflush(stdout);
	printf("Addr of Write16: 0x%08x\n", gCrateServerCBFcn.Write16);
	fflush(stdout);
	printf("Addr of Read32: 0x%08x\n", gCrateServerCBFcn.Read32);
	fflush(stdout);
	printf("Addr of Write32: 0x%08x\n", gCrateServerCBFcn.Write32);
	fflush(stdout);
