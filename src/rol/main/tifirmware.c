/*
  tifirmware.c - copy from Brian's tidEMload.c with small changes

  usage example for both vxWorks and UNIX:
...
/*     goto CLOSE; */
  ...
  printf(" Board Serial Number from PROM usercode is: 0x%08x \n", BoardSerialNumber);
  ...
  if (!((BoardSerialNumber&0xffff0000) == 0x71000000))
	...
	goto CLOSE;
      ...
      printf(" The board serial number will be set to: 0x%08x \n",BoardSerialNumber);
    }
