...
STATUS faInit(UINT32 addr, UINT32 addr_inc, int nadc, int iFlag) {
			...
			if ((rdata & FA_BOARD_MASK) != FA_BOARD_ID) {
				...
				if ((boardID <= 0) || (boardID > 21)) {
					...
					printf("Initialized FADC %2d  Slot # %2d at address 0x%08x (0x%08x) \n", nfadc, fadcID[nfadc],
							(UINT32) FAp[(fadcID[nfadc])], (UINT32) FAp[(fadcID[nfadc])] - fadcA24Offset);
				}
			}
			nfadc++;
	...
	if (!noBoardInit) {
		...
		for (ii = 0; ii < nfadc; ii++) {
			vmeWrite32(&(FAp[fadcID[ii]]->reset), (FA_RESET_ADC_FPGA1 | FA_RESET_ADC_FIFO1 |
	...
	for (ii = 0; ii < nfadc; ii++) {
		...
		if (!noBoardInit) {
			vmeWrite32(&(FAp[fadcID[ii]]->ctrl1), vmeRead32(&(FAp[fadcID[ii]]->ctrl1)) | (srSrc | trigSrc));
		}
	}
	...
	if (nfadc > 1) {
		...
		if (!noBoardInit) {
			for (ii = 0; ii < nfadc; ii++) {
				...
				vmeWrite32(&(FAp[fadcID[ii]]->adr_mb), (a32addr + FA_MAX_A32MB_SIZE) + (a32addr >> 16) + FA_A32_ENABLE);
			}
		}
		...
		fadcMinSlot = minSlot;
		if (!noBoardInit) {
			vmeWrite32(&(FAp[minSlot]->ctrl1), vmeRead32(&(FAp[minSlot]->ctrl1)) | FA_FIRST_BOARD);
			vmeWrite32(&(FAp[maxSlot]->ctrl1), vmeRead32(&(FAp[maxSlot]->ctrl1)) | FA_LAST_BOARD);
		}
	}
...
int faSetClockSource(int id, int clkSrc) {
	...
	/* Enable Clock source - Internal Clk enabled by default */
	vmeWrite32(&(FAp[id]->ctrl1), (vmeRead32(&FAp[id]->ctrl1) & ~(FA_REF_CLK_MASK)) | (clkSrc | FA_ENABLE_INTERNAL_CLK));
	taskDelay(20);
...
void faStatus(int id, int sflag) {
	...
	unsigned int adcStat[3], adcConf[3], PTW, PL, NSB, NSA, NP, adcChanDisabled, playbackMode;
 ...
 */
int faSetProcMode(int id, int pmode, unsigned int PL, unsigned int PTW, unsigned int NSB, unsigned int NSA, unsigned int NP, int bank) {
	...
	vmeWrite32(&(FAp[id]->adc_config[0]), ((pmode - 1) | (NP << 4)));
	...
	vmeWrite32(&(FAp[id]->ptw_max_buf), ptw_max_buf);
	vmeWrite32(&(FAp[id]->ptw_last_adr), ptw_last_adr);
	/* Enable ADC processing */
	vmeWrite32(&(FAp[id]->adc_config[0]), ((pmode - 1) | (NP << 4) | FA_ADC_PROC_ENABLE));
...
void faGSetProcMode(int pmode, unsigned int PL, unsigned int PTW, unsigned int NSB, unsigned int NSA, unsigned int NP, int bank) {
	int ii, res;
 ...
 *
 */
static void faWaitForAdcReady(int id) {
	int iwait = 0;
...
int faReadBlock(int id, volatile UINT32 *data, int nwrds, int rflag) {
	...
	if (rmode >= 1) { /* Block Transfers */
		...
		if (async) { /* Asynchonous mode - return immediately - don't wait for done!! */
			FAUNLOCK;
			return (OK);
		} else {
		...
		while (ii < nwrds) {
			...
			val = LSWAP(val);
#endif
			if ((val & FA_DATA_TYPE_DEFINE) && ((val & FA_DATA_TYPE_MASK) == FA_DATA_BLOCK_TRAILER))
				break;
			ii++;
		}
		ii++;
...
int faEnableInteralPlaybackTrigger(int id) {
	...
	FALOCK;
	vmeWrite32(&FAp[id]->ctrl1, (vmeRead32(&FAp[id]->ctrl1) & ~FA_TRIG_MASK) | FA_TRIG_VME_PLAYBACK);
	FAUNLOCK;
...
void faEnableSoftTrig(int id) {
	...
	/* Clear the source */
	FALOCK;
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) & ~FA_TRIG_MASK);
	/* Set Source and Enable*/
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) | (FA_TRIG_VME | FA_ENABLE_SOFT_TRIG));
	FAUNLOCK;
}
...
void faDisableSoftTrig(int id) {
	...
	FALOCK;
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) & ~FA_ENABLE_SOFT_TRIG);
	FAUNLOCK;
...
void faEnableSoftSync(int id) {
	...
	/* Clear the source */
	FALOCK;
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) & ~FA_SRESET_MASK);
	/* Set Source and Enable*/
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) | (FA_SRESET_VME | FA_ENABLE_SOFT_SRESET));
	FAUNLOCK;
}
...
void faDisableSoftSync(int id) {
	...
	FALOCK;
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) & ~FA_ENABLE_SOFT_SRESET);
	FAUNLOCK;
...
void faEnableClk(int id) {
	...
	FALOCK;
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) | (FA_REF_CLK_INTERNAL | FA_ENABLE_INTERNAL_CLK));
	FAUNLOCK;
...
void faDisableClk(int id) {
	...
	FALOCK;
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) & ~FA_ENABLE_INTERNAL_CLK);
	FAUNLOCK;
...
void faEnableTriggerOut(int id, int output) {
	...
	if (output > 2) {
		logMsg("faEnableTriggerOut: ERROR: output (%d) out of range.  Must be less than 3", output, 2, 3, 4, 5, 6);
		return;
	...
	FALOCK;
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) | bitset);
	FAUNLOCK;
...
void faGEnableBusError() {
	...
	FALOCK;
	for (ii = 0; ii < nfadc; ii++) {
		vmeWrite32(&(FAp[fadcID[ii]]->ctrl1), vmeRead32(&(FAp[fadcID[ii]]->ctrl1)) | FA_ENABLE_BERR);
	}
	FAUNLOCK;
...
void faEnableMultiBlock(int tflag) {
	...
	for (ii = 0; ii < nfadc; ii++) {
		id = fadcID[ii];
		FALOCK;
		vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) | mode);
		FAUNLOCK;
		faDisableBusError(id);
		if (id == fadcMinSlot) {
			FALOCK;
			vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) | FA_FIRST_BOARD);
			FAUNLOCK;
		}
		if (id == fadcMaxSlot) {
			FALOCK;
			vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) | FA_LAST_BOARD);
			FAUNLOCK;
...
void faDisableMultiBlock() {
	...
	FALOCK;
	for (ii = 0; ii < nfadc; ii++)
		vmeWrite32(&(FAp[fadcID[ii]]->ctrl1), vmeRead32(&(FAp[fadcID[ii]]->ctrl1)) & ~FA_ENABLE_MULTIBLOCK);
	FAUNLOCK;
...
int faSetClkSource(int id, int source) {
	...
	FALOCK;
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) & ~FA_REF_CLK_SEL_MASK);
	if ((source < 0) || (source > 7))
		source = FA_REF_CLK_INTERNAL;
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) | source);
...
int faSetTrigSource(int id, int source) {
	...
	FALOCK;
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) & ~FA_TRIG_SEL_MASK);
	if ((source < 0) || (source > 7))
...
int faSetSyncSource(int id, int source) {
	...
	FALOCK;
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) & ~FA_SRESET_SEL_MASK);
	if ((source < 0) || (source > 7))
...
void faEnableFP(int id) {
	...
	vmeWrite32(&(FAp[id]->ctrl1),
			vmeRead32(&(FAp[id]->ctrl1)) & ~(FA_TRIG_SEL_MASK | FA_SRESET_SEL_MASK | FA_ENABLE_SOFT_SRESET | FA_ENABLE_SOFT_TRIG));
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&(FAp[id]->ctrl1)) | (FA_TRIG_FP_ISYNC | FA_SRESET_FP_ISYNC));
	FAUNLOCK;
...
int faSetTrigOut(int id, int trigout) {
	...
	FALOCK;
	vmeWrite32(&(FAp[id]->ctrl1), (vmeRead32(&(FAp[id]->ctrl1)) & ~FA_TRIGOUT_MASK) | trigout << 12);
	FAUNLOCK;
...
int faSetThreshold(int id, unsigned short tvalue, unsigned short chmask) {
	...
	for (ii = 0; ii < FA_MAX_ADC_CHANNELS; ii++) {
		if (ii % 2 == 0) {
			...
			if (doWrite)
				vmeWrite32((unsigned int *) &(FAp[id]->adc_thres[ii]), lovalue << 16 | hivalue);
...
int faSetDAC(int id, unsigned short dvalue, unsigned short chmask) {
	...
	for (ii = 0; ii < FA_MAX_ADC_CHANNELS; ii++) {
		...
		if (ii % 2 == 0) {
			...
			if (doWrite)
				vmeWrite32((unsigned int *) &(FAp[id]->dac[ii]), lovalue << 16 | hivalue);
	...
	for (ii = 0; ii < FA_MAX_ADC_CHANNELS; ii++) {
		if (ii % 2 == 0) {
			...
			if ((1 << ii) & chmask)
				...
				lovalue &= FA_THR_VALUE_MASK;

			if ((1 << (ii + 1)) & chmask)
				hivalue |= FA_THR_IGNORE_MASK;
			else
				hivalue &= FA_THR_VALUE_MASK;
...
int faSetMGTSettings(int id, int txpre, int txswing, int rxequ) {
	...
	val = ((txpre & 0x1F) << 0) | ((txswing & 0xF) << 10) | ((rxequ & 0x3) << 14);
...
void faDataDecode(unsigned int data) {
	...
	case 7: /* PULSE INTEGRAL */
		...
		if (i_print)
			printf("%8X - PULSE INTEGRAL - chan = %d   pulse # = %d   quality = %d   integral = %d\n", data, fadc_data.chan,
					fadc_data.pulse_num, fadc_data.quality, fadc_data.integral);
		break;
	case 8: /* PULSE TIME */
		fadc_data.chan = (data & 0x7800000) >> 23;
		fadc_data.pulse_num = (data & 0x600000) >> 21;
	...
	case 9: /* STREAMING RAW DATA */
		if (fadc_data.new_type) {
			...
			if (i_print)
				printf("%8X - STREAMING RAW DATA - ena A = %d  chan A = %d   ena B = %d  chan B = %d\n", data, fadc_data.source_a,
						fadc_data.chan_a, fadc_data.source_b, fadc_data.chan_b);
		} else {
			fadc_data.valid_1 = 1;
			fadc_data.valid_2 = 1;
			fadc_data.adc_1 = (data & 0x1FFF0000) >> 16;
	...
	case 11: /* INTERNAL TRIGGER WORD */
		...
		if (i_print)
			printf("%8X - INTERNAL TRIGGER - type = %d   state = %d   num = %d   error = %d\n", data, fadc_data.trig_type_int,
					fadc_data.trig_state_int, fadc_data.evt_num_int, fadc_data.err_status_int);
...
void faTestSetSystemTestMode(int id, int mode) {
	...
	vmeWrite32(&(FAp[id]->ctrl1), vmeRead32(&FAp[id]->ctrl1) | reg);
...
int faGetSerialNumber(int id, char **rval, int snfix) {
	...
	if (sn[0] == FA_SERIAL_NUMBER_ACDI) { /* ACDI */
		strcpy(acdi_str, "");
	...
	{ /* ADV ASSEM */
		...
		if ((sn[0] == FA_SERIAL_NUMBER_ADV_MNFID1) && ((sn[1] & FA_SERIAL_NUMBER_ADV_MNFID2_MASK) == FA_SERIAL_NUMBER_ADV_MNFID2)) {
			strcpy(adv_str, "");
			for (ivme = 0; ivme < 3; ivme++) {
				for (ibyte = 3; ibyte >= 0; ibyte--) {
					...
					if (snfix > 0) { /* If needed, Add '-' after the B21595 */
						if (ivme == 1 && ibyte == 1) {
							if (byte != 0x2D) /* 2D = - */
							{
								strcat(adv_str, "-");
...
int faForceEndOfBlock(int id, int scalers) {
	...
	vmeWrite32(&FAp[id]->adc_config[0], proc_config & ~(FA_ADC_PROC_ENABLE));
...
void faSDC_Enable(int nsync) {
	...
	if (nsync != 0) /* FP triggers only */
		...
		/* both FP triggers and sync reset */
...
int faGetProcMode(int id, int *pmode, unsigned int *PL, unsigned int *PTW, unsigned int *NSB, unsigned int *NSA, unsigned int *NP) {
	unsigned int tmp;
