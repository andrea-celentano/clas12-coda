...
pthread_mutex_t   dsc2Mutex = PTHREAD_MUTEX_INITIALIZER;
#define DSCLOCK     pthread_mutex_lock(&dsc2Mutex);
#define DSCUNLOCK   pthread_mutex_unlock(&dsc2Mutex);
#endif



void
ttt123(int id, int chan, unsigned int thres)
{
  vmeWrite32(&dscp[id]->threshold[chan],thres);
  printf("returns 0x%08x\n",vmeRead32(&dscp[id]->threshold[chan]));
}
...
dsc2Status(UINT32 id, int pflag)
{
  ...
  printf("   TDC: 0x%02x (%d ns)   TRG: 0x%02x (%d ns)   TRG OUTPUT: 0x%x (%d ns)\n",
	 ...
	 4*(((pulsewidth&DSC_PULSEWIDTH_TRG_OUTPUT_MASK)>>28)+1));
 ...
 *
 *   Parameters :  UINT32 id    - Module slot number
 *                 UINT16 val   - Pulse width in ns (S.P.)
 *                                from 4ns to 40ns for TDC and TRG Pulsers
 *                                from 4ns to 64ns for TRG Output
 *                (S.P.: width was in units = 1ns for TDC and TRG
 *                                          = 4ns for TRG Output)
...
dsc2SetPulseWidth(UINT32 id, UINT16 val, UINT16 type)
{
  ...
  if(type==1 || type==2)
    {
      if(val > 40)
	{
	  printf("%s: ERROR: val=%d greater than maximum allowed 40.\n",__FUNCTION__,val);
	  return ERROR;
	}
    }
  else if(type==3)
    {
      if(val > 64)
	{
	  printf("%s: ERROR: val=%d greater than maximum allowed 64.\n",__FUNCTION__,val);
	  return ERROR;
	}
    }
  ...
  switch(type)
    {
    case 1:  /* TDC */
      dsc2SetTDCPulserWidth(dscp[id],val);
      break;
    case 2:  /* TRG */
    default:
      dsc2SetTRGPulserWidth(dscp[id],val);
      break;
    case 3:  /* TRGOUTPUT */
      dsc2SetTRGOutputWidth(dscp[id],val);
    }
...
/****************************************************************************
 * from 4ns to 40ns - for  tdcVal and trgVal
 * from 4ns to 64ns - for  trgoutVal
 */
int
 ...
 *                                                    
 *   Parameters :  UINT32 id    - Module slot number
 *                 UINT16 val   - Delay in ns
 *                                from 0ns to 1016ns for Scaler
 *                                from 0ns to  508ns for TRG Output
...
dsc2SetDelay(UINT32 id, UINT16 val, UINT16 type)
{
  ...
  if(type==1 && val>1016)
    {
      printf("%s: ERROR: val=%d greater than maximum allowed for Scaler 1016ns\n",
	     __FUNCTION__,val);
      return ERROR;
    }
  ...
  if(type==2 && val>508)
    {
      printf("%s: ERROR: val=%d greater than maximum allowed for TRG Output 508ns\n",
	     __FUNCTION__,val);
      return ERROR;
    }
  ...
  switch(type)
    {
    case 1:  /* Scaler */
      dsc2SetScalerInputDelay(dscp[id],val);
      break;
    case 2:  /* TRG Output */
    default:
      dsc2SetTRGOutputDelay(dscp[id],val);
      break;
    }
...
dsc2UpdateFirmwareAll(const char *filename)
{
	...
	printf("Updating firmware on all units ...\n");fflush(stdout);
	for(i = 0; i < Ndsc; i++)
	{
		printf("Updating firmware on unit %d...", i);fflush(stdout);
	    ...
	    printf(" Reloading FPGA ...");fflush(stdout);
		dsc2ReloadFPGA(i);
        sleep(2);
		printf("done.\n");fflush(stdout);
...
examples from SP:
dsc2Init(0x10d10000,0x10000,30)
dsc2MonAll()
dsc2MonBoard(0)
...
dsc2Init(UINT32 addr, UINT32 addr_inc, int ndsc2)
{
  ...
  for (ii=0; ii<ndsc2; ii++)
  {
    ...
    else
    {
      ...
      DSCUNLOCK;
      if(boardID != DSC2_BOARD_ID && boardID != 0x445f4332)
      {
#ifdef VXWORKS
        unsigned int tmp1 = boardID;
        unsigned int tmp2 = DSC2_BOARD_ID;
#else
        unsigned int tmp1 = LSWAP(boardID);
        unsigned int tmp2 = LSWAP(DSC2_BOARD_ID);
#endif
        char *s1=(char *)&tmp1, *s2=(char *)&tmp2;
	    printf("%s: ERROR: Board ID does not match: 0x%08x(%4.4s) , expected 0x%08x(%4.4s)\n",
	       __FUNCTION__,boardID,s1,DSC2_BOARD_ID,s2);
	    dscp[ii] = NULL;
	    return(ERROR);
      }

      printf("%s: Found addressable board at addr=0x%x, Board ID=0x%08x\n",
			 __FUNCTION__,(UINT32) dscp[ii],boardID);
    }
...
dsc2InitGlobals()
{
  ...
  for(ii=0; ii<NBOARDS; ii++)
  {
    ...
    for(jj=0;jj<16;jj++)  dsc2_TRG_Threshold [ii][jj] = 40;
...
{
  if(0<=board_number && board_number<dsc2_NN)
  {
    DSC2_MON_PRINT(board_number,(board_number+1),kk);
  }
  else
    logMsg("%s: ERROR: DSC2 with number %d is not initialized \n",
	   (int)__FUNCTION__,board_number,3,4,5,6);
}
...
dsc2GetFirmwareRevMin(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = vmeRead32(&(dsc2->firmware)) & DSC2_FIRMWARE_REV_MIN;
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetFirmwareRevMaj(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = (vmeRead32(&(dsc2->firmware)) & DSC2_FIRMWARE_REV_MAJ) >> 8;
  DSCUNLOCK;
  return(rval);
}
...
{
  int kk;
  for(kk=0; kk<dsc2_NN; kk++)
  {
    dsc2SetUniTDCThreshold(dsc2_BaseAddr[kk], TDC_thresh);
    dsc2SetUniTRGThreshold(dsc2_BaseAddr[kk], TRG_thresh);
  }
  return(0);
}
...
dsc2GetTDCThreshold(UINT32 addr, int channel)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = vmeRead32(&(dsc2->threshold[channel])) & DSC2_TDC_THRESHOLD;
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetTRGThreshold(UINT32 addr, int channel)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = (vmeRead32(&(dsc2->threshold[channel])) & DSC2_TRG_THRESHOLD) >> 16;
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetTDCPulserWidth(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = vmeRead32(&(dsc2->width)) & DSC2_TDC_PULSER_WIDTH;
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetTRGPulserWidth(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = (vmeRead32(&(dsc2->width)) & DSC2_TRG_PULSER_WIDTH) >> 16;
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetTRGOutputWidth(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = (((vmeRead32(&(dsc2->width)) & DSC2_TRG_OUTPUT_WIDTH) >> 28)+1)*4;
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetTDCmask(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = vmeRead32(&(dsc2->chMask)) & DSC2_TDC_MASK;
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetTRGmask(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = (vmeRead32(&(dsc2->chMask)) & DSC2_TRG_MASK) >> 16;
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetORmaskTDC(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = vmeRead32(&(dsc2->orMask)) & DSC2_TDC_MASK;
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetORmaskTRG(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = (vmeRead32(&(dsc2->orMask)) & DSC2_TRG_MASK) >> 16;
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetScalerInputDelay(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = (vmeRead32(&(dsc2->delay)) & DSC2_SCALER_INPUT_DELAY)*8;
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetTRGOutputDelay(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = ((vmeRead32(&(dsc2->delay)) & DSC2_TRG_OUTPUT_DELAY) >> 16)*4;
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetTRGscaler(UINT32 addr, int channel)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = vmeRead32(&(dsc2->trgScaler[channel]));
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetTDCscaler(UINT32 addr, int channel)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = vmeRead32(&(dsc2->tdcScaler[channel]));
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetTRGscalerVME(UINT32 addr, int channel)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = vmeRead32(&(dsc2->trgVmeScaler[channel]));
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetTDCscalerVME(UINT32 addr, int channel)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = vmeRead32(&(dsc2->tdcVmeScaler[channel]));
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetREFscalerVME(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = vmeRead32(&(dsc2->refVmeScaler));
  DSCUNLOCK;
  return(rval);
}
...
dsc2GetREFscalerGATE(UINT32 addr)
{
  ...
  volatile DSC2 *dsc2 = (DSC2 *) addr;
  DSCLOCK;
  rval = vmeRead32(&(dsc2->refGatedScaler));
  DSCUNLOCK;
  return(rval);
}
...
void
dsc2PrnAddrByNumber(int board_number)
{
  UINT32 addr = 0xffffffff;
  if((board_number >= 0) && (board_number < dsc2_NN))
  {
    printf("dsc2GetAddrByNumber(%d) = 0x%08x \n",
	   board_number, dsc2GetAddrByNumber(board_number));
    printf("dsc2_BaseAddr[%d] = 0x%08x \n",
 	   board_number, dsc2_BaseAddr[board_number]);
  }
  else
    printf("Board number %d not activated, addr = 0x%08x \n",
	   board_number, addr);
}
