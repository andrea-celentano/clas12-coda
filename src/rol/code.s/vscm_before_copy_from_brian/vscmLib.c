...
#define myDelay(ticks) taskDelay(ticks)
...
fssrStatus(int id, int chip)
{
  ...
  vscmDisableChanScalers(id, chip);	
...
void
vscmDisableChanScalers(int id, int chip)
{
  if (vscmIsNotInit(&id, __func__))
    return;
...
void
vscmClearChanScalers(int id, int chip)
{
  int i;
...
int
vscmReadChanScalers(int id, int chip, uint32_t *arr)
{
  int i;
...
vscmPrestart(char *fname)
{
  ...
  for(ii=0; ii<nvscm; ii++)
  {
    /* change ID for PP4 modules - remove it later !!!!!!!!!!!!!!!!!!! */
...
/* flag bit0=0-external clock, 1-internal */
int
  ...
  else
  {
    ...
    for(i=0; i<numvscm; i++)
    {
	  ...
	  */
	  ...
	  /*
      if (vmeaddr >= 21<<19 && numvscm != 1) break;
	  */
    ...
    for(i=0; i<nvscm; i++)
    {
      ...
      vmeWrite32(&VSCMpr[boardID]->DACTrigger, IO_MUX_0 | 0x80000000 | (0<<16)); /* disable trig to DAC just in case */ 
      ...
      /* Enable Bus Error (to diable, use '0' as last par) */
...
int
vscmSetDACTriggerSource(int id)
{
  int boardID;
  ...
  /* from vscmInit(): set trigger source for DAC to internal pulser */
  vmeWrite32(&VSCMpr[id]->DACTrigger, IO_MUX_PULSER | 0x80000000 | (0<<16));
