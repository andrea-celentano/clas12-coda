...
ctpInit(int flag)
{
  ...
  version[U24] = vmeRead32(&CTPp->fpga24.status2) & CTP_FPGA_STATUS2_FIRMWARE_VERSION_MASK;
  CTPUNLOCK;

  /* Check for consistent project firmware */
  if( ((version[U1] & CTP_FIRMWARE_PROJECT_MASK) != (version[U3] & CTP_FIRMWARE_PROJECT_MASK))
      || ((version[U1] & CTP_FIRMWARE_PROJECT_MASK) != (version[U24] & CTP_FIRMWARE_PROJECT_MASK))
      || ((version[U24] & CTP_FIRMWARE_PROJECT_MASK) != (version[U3] & CTP_FIRMWARE_PROJECT_MASK)))
    {
      printf("%s: ERROR: Inconsistent Project Firmware between FPGAs:\n",__FUNCTION__);
      for(ifpga = U1; ifpga<NFPGA; ifpga++)
	{
	  printf("  %3s: 0x%04x\n",sfpga[ifpga],version[ifpga]);
	}
      return ERROR;
    }

  /* Check for compatible project firmware */
  if( (version[U1] & CTP_FIRMWARE_PROJECT_MASK) != 
      (CTP_SUPPORTED_FIRMWARE & CTP_FIRMWARE_PROJECT_MASK) )
    {
      if(fIgnoreVersion)
	{
	  printf("%s: WARN: CTP Project Firmware Version (0x%x) not supported by this driver.\n",
		 __FUNCTION__,(version[U1] & CTP_FIRMWARE_PROJECT_MASK)>>12);
	  printf("           Project Firmware version 0x%x required. (Ignored)\n",
		 CTP_SUPPORTED_FIRMWARE>>12);
	}
      else
	{
	  printf("%s: ERROR: CTP Project Firmware Version (0x%x) not supported by this driver.\n",
		 __FUNCTION__,(version[U1] & CTP_FIRMWARE_PROJECT_MASK)>>12);
	  printf("           Project Firmware version 0x%x required.\n",
		 CTP_SUPPORTED_FIRMWARE>>12);
	  return ERROR;
	}
    }
  ...
  for(ifpga = U1; ifpga<NFPGA; ifpga++)
    {
      if((version[ifpga] & CTP_FIRMWARE_REVISION_MASK) < 
	 (CTP_SUPPORTED_FIRMWARE & CTP_FIRMWARE_REVISION_MASK))
	{
	  if(fIgnoreVersion)
	    {
	      printf("%s: WARN: CTP Firmware Version (0x%x) not supported by this driver.\n",
		     __FUNCTION__,(version[U1] & CTP_FIRMWARE_REVISION_MASK));
	      printf("           Firmware version 0x%x required. (Ignored)\n",
		     CTP_SUPPORTED_FIRMWARE & CTP_FIRMWARE_REVISION_MASK);
	    }
	  else
	    {
	      printf("%s: ERROR: CTP Firmware Version (0x%x) not supported by this driver.\n",
		     __FUNCTION__,(version[U1] & CTP_FIRMWARE_REVISION_MASK));
  ...
  printf("%s: CTP (Version: 0x%04x) initialized at Local Base address 0x%lx\n",
	 __FUNCTION__,
	 (version[U1]) & CTP_SUPPORTED_FIRMWARE_MASK,
	 (unsigned long) CTPp);
...
ppArray2vsArray(int *inarray)
{
  ...
  for(iport=1; iport<=16; iport++)
    {
      vmeslot = vxsPayloadPort2vmeSlot(iport);
