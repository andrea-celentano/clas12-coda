...
/* All the rest */
volatile struct v1495_struct *v1495 = 0;
unsigned int v1495_addr; //local
unsigned int v1495_addr_vme; //vme

pthread_mutex_t c1495_mutex = PTHREAD_MUTEX_INITIALIZER;
#define LOCK_1495 {				\
		if(pthread_mutex_lock(&c1495_mutex)<0)	\
		perror("pthread_mutex_lock");	\
}
#define UNLOCK_1495 {				\
		if(pthread_mutex_unlock(&c1495_mutex)<0)	\
...
/*****************************************************************************
 ...
 baseaddr: board address user space
...
int v1495firmware(unsigned int baseaddr, char *filename, int page, int user_vme) {
	...
	unsigned short *fw = (unsigned short *) (baseaddr + 0x800C);
	...
	fw_vers = vmeRead16(fw);
	printf("\nFIRMWARE DONE. VME FIRMWARE VERSION: 0x%08x\n", fw_vers);
...
int v1495Init(unsigned int addr, int iFlag) {
	...
	if (res != 0) {
		printf("v1495Init: ERROR in sysBusToLocalAdrs(0x39,0x%x,&laddr) \n", addr);
		return (ERROR);
	}
#else
	res = vmeBusToLocalAdrs(0x39, (char *) addr, (char **) &laddr);
	if (res != 0) {
		printf("v1495Init: ERROR in vmeBusToLocalAdrs(0x39,0x%x,&laddr) \n", addr);
		return (ERROR);
	}
#endif

	v1495_addr = laddr; //local
	v1495_addr_vme = addr; //vme
	v1495 = (struct v1495_struct *) (v1495_addr);

	/* Check if Board exists at that address */
#ifdef VXWORKS
	res = vxMemProbe((char *) &(v1495->firmwareRev),0,2,(char *)&rdata);
#else
	res = vmeMemProbe((char *) &(v1495->firmwareRev), 2, (char *) &rdata);
#endif

	if (res < 0) {
		printf("v1495Init: ERROR: No addressable board at addr=0x%x\n", (UINT32) v1495);
		v1495 = NULL;

		return (ERROR);
	} else {
		/* Check if this is a Model 1495 */
		printf("v1495 addr ok: VME 0x%08x LOCAL 0x%08x \n", v1495_addr_vme, v1495_addr);
		printf("rdata: 0x%08x \n", rdata);

		boardID = ((vmeRead16(&(v1495->configROM[0x2C])) & (0xff)) << 16) + ((vmeRead16(&(v1495->configROM[0x28])) & (0xff)) << 8)
				+ (vmeRead16(&(v1495->configROM[0x24])) & (0xff));

		if (boardID != V1495_BOARD_ID) {
			printf("v1495Init: ERROR: Board ID does not match: 0x%x expected 0x%x \n", boardID, V1495_BOARD_ID);
			//return (ERROR);
		}

		boardID = vmeRead16((volatile unsigned short*) (v1495_addr + 0x103C));
		printf("v1495Init: firmware ID 0x%08x \n", boardID);

	}

	/* Check if we are to exit when pointers are setup */
	noBoardInit = (iFlag & V1495_INIT_SKIP) >> 16;

	printf("v1495 init done \n");
	return OK;
}
...
unsigned int v1495ReadChannelScaler(int scaler, int clear, int latch) {
	...
	unsigned short counterL, counterH;
	unsigned short mask;
	unsigned int counter;
	if (v1495 == 0) {
		printf("v1495ReadChannelScaler Error! v1495 not initialized \n");
		return 0;
	}
	if ((scaler < 0) || (scaler >= V1495_NSCALERS_CHANNEL)) {
		printf("c1495ReadChannelScaler ERROR, scaler number error %d \n", scaler);
	}
	if ((clear < 0) || (clear > 1)) {
		printf("c1495ReadChannelSaler ERROR clear\n");
	}
	if ((latch < 0) || (latch > 1)) {
		printf("c1495ReadChannelSaler ERROR latch\n");
	}

	mask = (scaler << 2) + (clear << 1) + latch;

	LOCK_1495;
	vmeWrite16((volatile unsigned short*) &(v1495->scalerChannelCtrl), mask);
	counterL = vmeRead16((volatile unsigned short*) &(v1495->scalerChannelDataL)) & 0xFFFF;
	counterH = vmeRead16((volatile unsigned short*) &(v1495->scalerChannelDataH)) & 0xFFFF;
	UNLOCK_1495;
...
void v1495PrintChannelScaler(int scaler, int clear, int latch) {
	...
	if (counter) {
		printf("v1495PrintChannelScaler: %i %u \n", scaler, counter);
	} else {
		printf("v1495PrintChannelScaler: %i returned 0\n", scaler);
	}
}
...
unsigned int v1495ReadLogicScaler(int scaler, int clear, int latch) {
	...
	unsigned int counter;
	if (v1495 == 0) {
		printf("v1495ReadLogicScaler Error! v1495 not initialized \n");
		return 0;
	}
	if ((scaler < 0) || (scaler >= V1495_NSCALERS_LOGIC)) {
		printf("c1495ReadLogicScaler ERROR, scaler number error %d \n", scaler);
	}
	if ((clear < 0) || (clear > 1)) {
		printf("c1495ReadChannelSaler ERROR clear\n");
	}
	...
	LOCK_1495;
	vmeWrite16((volatile unsigned short*) &(v1495->scalerLogicCtrl), mask);
	counterL = vmeRead16((volatile unsigned short*) &(v1495->scalerLogicDataL)) & 0xFFFF;
	counterH = vmeRead16((volatile unsigned short*) &(v1495->scalerLogicDataH)) & 0xFFFF;
	UNLOCK_1495;
...
void v1495PrintLogicScaler(int scaler, int clear, int latch) {
	...
	if (counter) {
		printf("v1495PrintLogicScaler: %i %u \n", scaler, counter);
	} else {
		printf("v1495PrintLogicScaler: %i returned 0\n", scaler);
	}
}
...
unsigned int v1495ReadTrgScaler(int scaler, int clear, int latch) {
	...
	unsigned int counter;
	if (v1495 == 0) {
		printf("v1495ReadTrgScaler Error! v1495 not initialized \n");
		return 0;
	}
	if ((scaler < 0) || (scaler >= V1495_NSCALERS_LOGIC)) {
		printf("c1495ReadTrgScaler ERROR, scaler number error %d \n", scaler);
	}
	if ((clear < 0) || (clear > 1)) {
		printf("c1495ReadChannelSaler ERROR clear\n");
	}
	...
	LOCK_1495;
	vmeWrite16((volatile unsigned short*) &(v1495->scalerTrgCtrl), mask);
	counterL = vmeRead16((volatile unsigned short*) &(v1495->scalerTrgDataL)) & 0xFFFF;
	counterH = vmeRead16((volatile unsigned short*) &(v1495->scalerTrgDataH)) & 0xFFFF;
	UNLOCK_1495;
...
void v1495PrintTrgScaler(int scaler, int clear, int latch) {
	...
	if (counter) {
		printf("v1495PrintTrgScaler: %i %u \n", scaler, counter);
	} else {
		printf("v1495PrintTrgScaler: %i returned 0\n", scaler);
	}
}
...
/**************************************************************************************
 *
 *  v1495ReadAllChannelsScaler - Scaler Data readout routine
 ...
 */
int v1495ReadAllChannelScaler(volatile unsigned int *data, unsigned int chmask, int rflag) {
	int doLatch = 0, doClear = 0, ichan = 0;
	int dCnt = 0;

	if (rflag & ~(V1495_SCALER_CTRL_MASK)) {
		logMsg("v1495ReadAllChannelScaler: WARN : rflag (0x%x) has undefined bits \n", rflag, 0, 0, 0, 0, 0);
	}
	...
	for (ichan = 0; ichan < V1495_NSCALERS_CHANNEL; ichan++) {
		if ((1 << ichan) & chmask) {
			data[dCnt] = v1495ReadChannelScaler(ichan, doClear, doLatch);
			dCnt++;
		}
	}
...
int v1495ReadAllLogicScaler(volatile unsigned int *data, unsigned int chmask, int rflag) {
	...
	if (rflag & ~(V1495_SCALER_CTRL_MASK)) {
		logMsg("v1495ReadAllLogicScaler: WARN : rflag (0x%x) has undefined bits \n", rflag, 0, 0, 0, 0, 0);
	}
	...
	for (ichan = 0; ichan < V1495_NSCALERS_LOGIC; ichan++) {
		if ((1 << ichan) & chmask) {
			data[dCnt] = v1495ReadLogicScaler(ichan, doClear, doLatch);
			dCnt++;
		}
	}
...
int v1495ReadAllTrgScaler(volatile unsigned int *data, unsigned int chmask, int rflag) {
	...
	if (rflag & ~(V1495_SCALER_CTRL_MASK)) {
		logMsg("v1495ReadAllTrgScaler: WARN : rflag (0x%x) has undefined bits \n", rflag, 0, 0, 0, 0, 0);
	}
	...
	for (ichan = 0; ichan < V1495_NSCALERS_LOGIC; ichan++) {
		if ((1 << ichan) & chmask) {
			data[dCnt] = v1495ReadTrgScaler(ichan, doClear, doLatch);
			dCnt++;
		}
	}
//	printf("v1495ReadAllLogicScaler done: %i \n",dCnt);
