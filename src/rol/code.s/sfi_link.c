...
int ts_sfi_events(void);
void write_aux(long data);
volatile unsigned long read_aux(void);
void acknowledge_sfi(void);
int ran_sfi(void);
void print_bit_sfi(struct b_field *bit);

int int_level_sfi = INTERRUPT_LEVEL;
...
sfi_link()
{
	...
	id_1 = sp(ts_sfi_events,0,0,0,0,0,0,0,0,0);		/*  spawn ts event process  */
...
/*  reconstruct TS memory data  ---   all trigger patterns that form the memory address were 
	...
	for( addr = 1; addr <= 4095;  addr++ )		/*  assign data to all memory addresses  */
	{
		do
		{	
			x = ran_sfi() & 0x3FFF;		/*  choose random data for memory bits  */
...
/*  set interrupt level, & assign interrupt ID = 0xEE  */
	printf("\n set interrupt level, & assign interrupt ID = 0xEE \n");
	*int_id_w = 0x8EE + (int_level_sfi << 8);
...
/*  connect routine to interrupt  */
	...
	status = intConnect(INUM_TO_IVEC(0xEE), (FUNCPTR)acknowledge_sfi, 0);
...
/*  on interrupt, read & test data, write acknowledge and increment count  */
{
	...
	*int_mask_w = 0x1000;			/* clear interrupt flag */
	sysBusIntAck(int_level_sfi); 		/* acknowledge VME interface chip */
	...
	{						
		trig_hit = ran_sfi() & 0xFFF & ACTIVE;	/*  use only active trigger inputs  */
	}  while( trig_hit == 0 );			/*  require non-zero pattern  */

	if( sfi_version == 1)
	    trig_data = (( memory[trig_hit] & 0x0F0000 ) >> 16) & 0xf; /* 4 bits of event type only */
	else    
	    trig_data = (( memory[trig_hit] & 0x3F0000 ) >> 16) & 0xf; /* 6 bits of event type only */
