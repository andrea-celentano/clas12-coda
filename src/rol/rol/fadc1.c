...
/* on dvcs2: goto '$CODA/src/rol' and type 'make urols' */
/* FX TAGGED HERE */
...
changeFadcMode()
{
  ...
  {
    fadc_mode=7;
    printf("set fadc_mode to %d (pulse integral mode with tdc)\n",fadc_mode);
  }
  else
  {
    fadc_mode=1;
...
#define FADC_WINDOW_LAT      300/*325*//*850*/  /* Trigger Window Latency */
...
extern   int fadcA32Base;           /* This will need to be reset from it's default
                                     * so that it does not overlap with the TI */
