...
#ifdef STANDALONE
#ifdef WITHVME
#define INIT_NAME fbvmerol1_standalone__init
#else
...
*/
  ...
  {
	slotnums[ii] = tdc1190GetGeoAddress(ii);
    printf("[%d] slot %d\n",ii,slotnums[ii]);
  }
#endif
#endif
...
usrtrig(unsigned long EVTYPE, unsigned long EVSOURCE)
{
  long EVENT_LENGTH;
  int len;
  unsigned long i, ii, jj, datascan, cc;
  ...
  int nev, rlen, rlenbuf[NBOARDS], res;
  /*
...
logMsg("word_count=%d\n",word_count,2,3,4,5,6);
      ...
      for(jj=0; jj<nboards; jj++)
      {
        rlen = rlenbuf[jj];
        if(rlen <= 0) continue;
        for(i=0; i<rlen; i++) *rol->dabufp++ = *tdcbuf++;
	  }
#endif
#endif
...
#ifndef STANDALONE /* for standalone mode every event if sync !!! */
    if(syncFlag == 1) /* Check if we have a sync flag */
#endif
    {
      EIEIO;
      SynC;
