...
if(rol->pid == 0)
{
  ...
  for(ii=0; ii<nboards; ii++)
  {
    tdc1190SetEdgeDetectionConfig(ii, 0x3);
  }
  NBsubtract = 5;
}
...
if(rol->pid == 0)
{
  dsc2_TDC_width = 4; /*starting width*/
  for(ii=0; ii<9; ii++) dsc2SetTDCPulserWidth(dsc2GetAddrByNumber(ii),dsc2_TDC_width);
}
...
if(rol->pid == 0)
{
if(dsc2_TDC_width < 41) dsc2_TDC_width += 2;
for(ii=0; ii<9; ii++) dsc2SetTDCPulserWidth(dsc2GetAddrByNumber(ii),dsc2_TDC_width);
logMsg("SYNC reached: dsc2_TDC_width = %2d \n",dsc2_TDC_width,2,3,4,5,6);
}
