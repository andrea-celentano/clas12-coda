...
/* unlike smartsockets, activemq will implement 'application', 'unique_id' etc
as parts of topic/queue name; that name looks like this:

APPLICATION.UNIQUE_NAME.STATUS.XXX.YYY
APPLICATION.UNIQUE_NAME.XXX.YYY.ZZZ
...
/* input:
  uniq_dgrp - unique string (like 'dbrouter')
  id_str - description (like 'Database Router')
*/

int
ipc_init(const char *uniq_dgrp, const char *id_str)
{
  //T_IPC_MT mt;
  //T_CB cb;
  //T_OPTION opt;
  ...
  server.init(getenv("EXPID"), NULL, NULL, (char *)uniq_dgrp, NULL, "*");
...
ipc_close(void)
{
  ...
  //if(strcasecmp(disconnect_mode,"warm")==0) TipcSrvDestroy(T_IPC_SRV_CONN_WARM);
  //else                                      TipcSrvDestroy(T_IPC_SRV_CONN_NONE);
	       ...
	       int severity, const char *msgstatus, int code, const char *text)
{
  ...
  char *domain = (char *)"insert_msg";
  char *user   = getenv("USER");
  int msgtime  = time(NULL);
  char host[50];
  ...
  IpcServer &server = IpcServer::Instance();
  /*do not call 'server.init()' or 'server.close()', assumed it was called already*/
  server << clrm << "cmlog" << domain << host << user << msgtime << name << facility << process << msgclass << severity << msgstatus << code << text << endm;
...
int
ipc_set_application(const char *name)
{
...
int
ipc_set_user_status_poll_callback(/*void (*func)(T_IPC_MSG)*/)
{
...
int
ipc_set_control_message_callback(
...
int
ipc_set_quit_callback(void (*func)(int))
{
